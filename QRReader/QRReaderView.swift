...
public class QRReaderView: UIView {
    ...
    private func failure(_ error: QRReaderErrors) {
        delegate?.qrReaderView(self, didFailure: error)
    }

    private func success(_ code: String) {
        delegate?.qrReaderView(self, didSuccess: code)
    }

    private func didChangeTorchActive(isOn: Bool) {
        delegate?.qrReaderView(self, didChangeTorchActive: isOn)
    }
}
